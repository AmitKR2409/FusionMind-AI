...
from session_manager import init_session_state  # (optional, currently not used but fine)

def display_header():
    """Renders the main title and introductory markdown."""
    st.title("🤖 FusionMind AI")
            ...
            for event in trace_events:
                ...
                if event['node_name'] == 'rag_lookup' and 'sufficiency_verdict' in event['details']:
                    ...
                    if verdict == "Sufficient":
                        ...
                        st.warning(
                            f"**RAG Verdict:** {verdict} - No sufficient info in Knowledge Base. "
                            "Diverting to Web Search."
                        )
                    
                    if 'retrieved_content_summary' in event['details']:
                        st.markdown(
                            f"**Retrieved Content Summary:** "
                            f"`{event['details']['retrieved_content_summary']}`"
                        )
                elif event['node_name'] == 'web_search' and 'retrieved_content_summary' in event['details']:
                    st.markdown(
                        f"**Web Search Content Summary:** "
                        f"`{event['details']['retrieved_content_summary']}`"
                    )
