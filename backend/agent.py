...
def rag_node(state: AgentState, config: RunnableConfig) -> AgentState:
    print("\n--- Entering rag_node ---")
    query = next((m.content for m in reversed(state["messages"]) if isinstance(m, HumanMessage)), "")
    # Get web_search_enabled directly from the config
    ...
    # 🚑 IMPORTANT: be robust to Groq tool / structured-output failures
    try:
        verdict: RagJudge = judge_llm.invoke(judge_messages)
        print(f"RAG Judge verdict: {verdict.sufficient}")
    except Exception as e:
        # If judge fails (tool_use_failed, etc.), just assume the context is usable
        print(f"Judge LLM failed, defaulting to sufficient=True. Error: {e}")
        verdict = RagJudge(sufficient=True)
